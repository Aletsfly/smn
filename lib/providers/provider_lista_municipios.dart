...
class ProviderListaMunicipios with ChangeNotifier {
  ...
  List<ModeloMunicipio> get listaDeMunicipiosFiltrada => _lista_de_municipios_filtrada;
  ...
  Future<void> cargaMunicipios() async {
    ...
    try {
      if (_lista_de_municipios.isEmpty) {
        print('akiii');
        _lista_de_municipios = await ServicioCargaMunicipios().descargaMunicipios();
      }
  ...
  void filtrarMunicipios(String texto){
    if (texto.isEmpty){
      ...
      _lista_de_municipios_filtrada = _lista_de_municipios.where((municipio) => municipio.label.toLowerCase().contains(texto.toLowerCase())).toList();
    }
    notifyListeners();
  }

  Future<ModeloMunicipio> obetenerMunicipioPorNombre(String nombreDeMunicipio) async {
    if(_lista_de_municipios.isEmpty){
      await cargaMunicipios();
    }
    final ciudad = _lista_de_municipios.firstWhere(
        (municipio)=>municipio.label.toLowerCase().contains(nombreDeMunicipio.toLowerCase()),
        orElse: ()=>ModeloMunicipio(label: "Ciudad No Encontrada", idEdo: "", idMpo: "")
    );
